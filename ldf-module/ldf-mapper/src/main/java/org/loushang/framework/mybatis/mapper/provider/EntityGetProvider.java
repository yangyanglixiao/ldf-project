...
public class EntityGetProvider extends MapperTemplate {
	 ...
	 * @return
	 */
	public String getByEntity(MappedStatement ms) {
	...
	public String query(MappedStatement ms) {
		...
		for (EntityColumn column : table.getEntityClassColumns()) {
			...
			if ("sqlserver".equals(DBType)) {
				sql.append(" and ").append(column.getColumn()).append(" like ").append("'%#{")
						.append(column.getProperty()).append("}%'");
			} else if ("oracle".equals(DBType)) {
				sql.append(" and ").append(column.getColumn()).append(" like ").append("CONCAT(")
						.append("CONCAT('%',#{").append(column.getProperty()).append("}),'%')");
			} else {
				sql.append(" and ").append(column.getColumn()).append(" like ").append("CONCAT('%',#{")
						.append(column.getProperty()).append("},'%')");
			}
